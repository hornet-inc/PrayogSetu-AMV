...
deleteBtn.addEventListener("click", async () => {
    ...
    showPopup("Deleted", "Blogs link removed from Database");
});
...
async function loadGoogleSheet(url) {
    try {
        ...
        fileStatus.textContent = "Using Blog Post Data from Google Sheets";
        fileStatus.style.color = "#00ffea";

        googleSheetStatus.textContent = `Sheet: ${url}`;
        googleSheetStatus.style.color = "#00ffea";

        showPopup("Loaded", "Blog Post data loaded successfully!");
    } 
    catch (err) {
...
async function saveLinkToFirebase(url) {
    ...
    showPopup("Saved", "Blogs link saved successfully!");
}
