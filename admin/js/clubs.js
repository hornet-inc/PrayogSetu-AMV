...
deleteBtn.addEventListener("click", async () => {
    ...
    showPopup("Deleted", "Clubs link removed from Database");
});
...
async function loadGoogleSheet(url) {
    try {
        ...
        fileStatus.textContent = "Using Clubs Data from Google Sheets";
        fileStatus.style.color = "#00ffea";

        googleSheetStatus.textContent = `Sheet: ${url}`;
        googleSheetStatus.style.color = "#00ffea";

        showPopup("Loaded", "Clubs Data loaded successfully!");
    } 
    catch (err) {
...
async function saveLinkToFirebase(url) {
    ...
    showPopup("Saved", "Club Details link saved successfully!");
}
...
function populateTable(workbook) {
    ...
    rows.forEach((row, index) => {
        ...
        let fileURL = safe(row["File"]);
        let fileHTML = "";

        // If file URL exists → make VIEW clickable
        if (fileURL && fileURL.trim() !== "") {
            fileHTML = `<a href="${fileURL}" target="_blank" class="view-link">VIEW</a>`;
        } else {
            fileHTML = `<span style="opacity:0.4;">No File</span>`;
        }

        const tr = document.createElement("tr");
        tr.innerHTML = `
            <td>${index + 1}</td>
            <td>${safe(row["Club Name"])}</td>
