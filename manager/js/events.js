...
deleteBtn.addEventListener("click", async () => {
    ...
    showPopup("Deleted", "Events link removed from Database");
});
...
async function loadGoogleSheet(url) {
    try {
        ...
        fileStatus.textContent = "Using Event Timelines from Google Sheets";
        fileStatus.style.color = "#00ffea";

        googleSheetStatus.textContent = `Sheet: ${url}`;
        googleSheetStatus.style.color = "#00ffea";

        showPopup("Loaded", "Events Data loaded successfully!");
    } 
    catch (err) {
...
async function saveLinkToFirebase(url) {
    ...
    showPopup("Saved", "Events Details link saved successfully!");
}
